...
class MandelbrotExample(implicit p: Parameters) extends RoCC()(p) {
  val cx = Reg(UInt(width = 32))
  val cy = Reg(UInt(width = 32))
  val req_rd = Reg(io.resp.bits.rd)
  
  val result = Reg (init = UInt(0, width = 32))
  //val zx = Reg (UInt(width = xLen))
  //val zy = Reg (UInt(width = xLen))
  ...
  //val d = zx*zx + zy*zy
  //val zx_n = (zx*zx - zy*zy + cx*10000.U)/10000.U
  //val zy_n = (2.U*zx*zy + cy*10000.U)/10000.U
  ...
  when (state === s_cal) {
    state := s_resp
    result := px*px + py*py
    //repeat := repeat + UInt(1)
    //zx := zx_n
    //zy := zy_n
  }

  /*when (repeat > 9.U || d >= 400000000.U && state === s_cal){
    state := s_resp
  }*/
  io.resp.valid := (state === s_resp)
  io.resp.bits.rd := req_rd
